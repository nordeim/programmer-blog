...
Packages (apps/web/src/packages/**)
         @devlog/db, @devlog/auth, @devlog/email, @devlog/types, @devlog/config.
...
- **Schema** lives in `packages/db/src/schema.ts`. **7 tables:** `users`, `sessions`, `posts`, `tags`, `postsToTags`, `subscribers`, `comments`, `siteSettings`.
...
|---|---|---|
| `DATABASE_PATH` | SQLite file path (default `./devlog.db`) | No |
...
| `NEXT_PUBLIC_SITE_URL` | Canonical site URL (RSS, OG tags, email links) | No (default `http://localhost:3000`) |
